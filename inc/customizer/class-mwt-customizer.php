...
if (!class_exists('Iqconnetik_Customizer')) :

	class Iqconnetik_Customizer
	{
		...
		public function __construct($settings = array())
		{
			...
			$settings = apply_filters('iqconnetik_customizer_framework_settings', $settings);
			...
			foreach ($this->registered_settings as $setting_id => $setting) :
				...
				if (empty($active_callback) && !empty($setting['visible'])) {
					$active_callback = function ($control) {
						$settings = iqconnetik_customizer_settings_array();
				...
				switch ($setting['type']) {
						// dropdown-category
						// see Iqconnetik_Dropdown_Category_Control class
					case 'dropdown-category':
						$customizer->add_control(
							new Iqconnetik_Dropdown_Category_Control(
								$customizer,
								$setting_id,
								array(
									'active_callback' => $active_callback,
						...
						break;
						// block-heading
						// see Iqconnetik_Block_Heading_Control class
					case 'block-heading':
						$customizer->add_control(
							new Iqconnetik_Block_Heading_Control(
								$customizer,
								$setting_id,
								array(
									'active_callback' => $active_callback,
						...
						break;
						// hidden-option
						// see Iqconnetik_Hidden_Customize_Control class
					case 'hidden-option':
						$customizer->add_control(
							new Iqconnetik_Block_Heading_Control(
								$customizer,
						...
						break;
						// google-font
						// see Iqconnetik_Google_Font_Control class
					case 'google-font':
						$customizer->add_control(
							new Iqconnetik_Google_Font_Control(
								$customizer,
								$setting_id,
								array(
									'active_callback' => $active_callback,
					...
					case 'background-image':
						$customizer->add_control(
							new Iqconnetik_Background_Image_Customize_Control(
								$customizer,
								$setting_id,
								array(
									'active_callback' => $active_callback,
